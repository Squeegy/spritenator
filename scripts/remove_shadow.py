...
def remove_shadow(pil_img, shadow_threshold=50, edge_threshold=100):
    ...
    # Apply inverse thresholding to identify shadow
    # Changing the threshold type to THRESH_BINARY_INV
    _, shadow_mask = cv2.threshold(gray, shadow_threshold, 255, cv2.THRESH_BINARY_INV)

    # Combine the shadow mask with the dilated edges to preserve the outlines
    combined_mask = cv2.bitwise_or(shadow_mask, edges_dilated)

    # Create an alpha channel for transparency
    b, g, r, a = cv2.split(open_cv_image_bgr)
    a[combined_mask == 255] = 0  # Set alpha to 0 (transparent) where the shadow is detected
